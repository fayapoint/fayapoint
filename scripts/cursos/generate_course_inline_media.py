# -*- coding: utf-8 -*-
"""Leitura 2.0 — escala (17/07/2026): midia inline para os caps 2-30 do
chatgpt-zero (cap 1 = piloto ja feito). Por capitulo: 6 imagens Qwen
(4 inline + 2 bases de video) + 2 videos LTX I2V.
Regra do espelho ampliada: cenario = TEMA do modulo; acao = SECAO do
texto (slot) com tempero do TIPO de aula. Os trechos-alvo sao identicos
entre capitulos (curso templado), entao slot+tipo+cenario = espelho fiel.
...
# 6 temas do curso (mesmos do header) -> cenario + glow
THEMES = [
...
# Acao por slot (secao do texto) x tipo de aula (0..4):
# tipos: 0 Fundamentos · 1 Configuracao · 2 Aplicacao · 3 Erros/validacao · 4 Projeto/entrega
SLOT_ACTIONS = {
    "sistema": [  # apos Conceitos-Chave §1: "qualidade em IA e sistema em camadas"
    ...
    "intencao": [  # apos Conceitos-Chave §2: "separar intencao de execucao"
    ...
    "fluxo": [  # base do VIDEO apos Fluxo de Execucao (5 passos)
    ...
    "cenario": [  # apos Cenarios Aplicados §1: "de pedido vago a processo gerenciado"
    ...
    "validacao": [  # apos Erros Comuns: "IA nao e autoridade final"
    ...
    "dica": [  # base do VIDEO apos a Dica Pro: "checklist antes do prompt"
...
CAPS = range(2, 31)  # cap 1 = piloto ja feito
manifest = []
...
Path(__file__).with_name("course-inline-manifest.json").write_text(
    json.dumps(manifest, ensure_ascii=False, indent=1), encoding="utf-8")
print("COURSE INLINE MEDIA DONE", flush=True)
