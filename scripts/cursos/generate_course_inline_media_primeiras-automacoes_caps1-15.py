# -*- coding: utf-8 -*-
"""primeiras-automacoes caps 1-15 -- midia inline com prompts BESPOKE por capitulo
(nao lookup table generica). Reaproveita verbatim de
generate_course_inline_media_primeiras-automacoes.py: THEMES, FUSION, NEGATIVE,
build_image, build_video, upload, wait, post, free_vram, seed scheme.
So a logica de selecao de prompt muda: le
content_drafts/primeiras-automacoes_prompts_1-15.json (prompt de imagem e motion
de video escritos a mao, grounded no texto real de cada capitulo).
Saida: C:/WORKS/ComfyUI/output/course_media/primeiras-automacoes/inline/capNN-*
Roda concorrente com outro agente cobrindo caps 16-30 no mesmo servidor ComfyUI --
ranges de capitulo disjuntos, sem colisao de seed."""
...
PROMPTS_PATH = Path(__file__).parent / "content_drafts" / "primeiras-automacoes_prompts_1-15.json"
...
# 6 temas do curso (1 por modulo) -> cenario + glow -- reaproveitado VERBATIM do
# script de referencia do mesmo curso (generate_course_inline_media_primeiras-automacoes.py)
THEMES = [
...
CAPS = range(1, 16)  # este script cobre so os caps 1-15
...
PROMPTS = json.loads(PROMPTS_PATH.read_text(encoding="utf-8"))["caps"]
manifest = []
failures = []
...
for cap in CAPS:
    ...
    cap_prompts = PROMPTS[str(cap)]["images"]
    for si, slot in enumerate(SLOTS_IMG):
        name = f"cap{cap:02d}-{slot}"
        if list(OUT_DIR.glob(f"{name}_*.png")):
            continue  # retomavel
        bespoke = cap_prompts[slot]
        prompt_text = f"{setting}, {bespoke}, {accent} glow" + FUSION
    ...
    for pid, name in jobs:
        ...
        try:
            ...
            if pid in h and h[pid].get("status", {}).get("completed"):
                ...
                print(f"[img {name}] ERROR in queue: {json.dumps(h[pid]['status'])[:200]}", flush=True)
...
n_ok = 0
for cap in CAPS:
    cap_motions = PROMPTS[str(cap)]["video_motion"]
    for slot in SLOTS_VIDEO:
        ...
        manifest.append({"kind": "video", "name": name, "seed": seed, "ok": ok, "motion": motion})
...
print(f"{SLUG.upper()} CAPS 1-15 INLINE MEDIA DONE — failures: {len(failures)}", flush=True)
