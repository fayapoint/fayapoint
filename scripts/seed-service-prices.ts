...
const servicePrices: ServicePriceDocument[] = [
  ...
  },
  {
    ...
    description: "≤60s vertical edit with captions and motion graphics.",
    unitType: "per_project",
    minQuantity: 2,
    defaultQuantity: 4,
  ...
  try {
    ...
    for (const doc of servicePrices) {
      ...
      if (result.upsertedCount && result.upsertedCount > 0) {
        inserted += 1;
        console.log(`  ➕ Inserted ${doc.serviceSlug} - ${doc.unitLabel}`);
      } else if (result.modifiedCount > 0) {
        updated += 1;
        console.log(`  🔄 Updated ${doc.serviceSlug} - ${doc.unitLabel}`);
      }
    }
