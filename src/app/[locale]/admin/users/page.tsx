  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] border", getRoleBadgeColor(user.role))}>
          {user.role === "admin" && <Shield size={10} />}
          {user.role}
        </span>
        <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] border", getPlanBadgeColor(user.subscription?.plan))}>
          {user.subscription?.plan === "business" && <Crown size={10} />}
...
function UserRow({ user, onEdit, onDelete }: { user: User; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
      ...
      <td className="py-3 px-4">
        <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border", getRoleBadgeColor(user.role))}>
          {user.role === "admin" && <Shield size={10} />}
          {user.role}
        </span>
      </td>
      <td className="py-3 px-4">
        <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border", getPlanBadgeColor(user.subscription?.plan))}>
          {user.subscription?.plan === "business" && <Crown size={10} />}
          {user.subscription?.plan || "free"}
        </span>
      </td>
