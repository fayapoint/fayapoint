  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-secondary border border-border mb-6">
              ...
              <span className="text-sm text-amber-200">{t("hero.badge")}</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-lg mx-auto">
              <motion.div
                ...
                className="bg-secondary backdrop-blur-sm border border-border rounded-xl p-4"
              >
                <ImageIcon className="w-6 h-6 text-amber-400 mx-auto mb-2" />
                <div className="text-2xl font-bold text-white">
                  {stats?.totalCreations?.toLocaleString() || '—'}
                </div>
                <div className="text-xs text-muted-foreground">{t("stats.creations")}</div>
              </motion.div>
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">{t("stats.todayCreations")}</div>
              </motion.div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20">
              ...
              <Link 
                href="/portal" 
                className="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-amber-600 hover:bg-amber-700 rounded-lg transition-colors"
              >
                <Wand2 size={18} />
                {t("cta.button")}
              </Link>
              ...
              {hasMore && (
                <div className="text-center mt-10">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-8 py-3 bg-secondary hover:bg-white/10 border border-border rounded-full transition-colors disabled:opacity-50"
                  >
                    {isLoadingMore ? (
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {steps.map((step, idx) => (
              ...
              >
                ...
                <div className="text-sm text-amber-400 mb-2">0{idx + 1}</div>
                <h3 className="text-xl font-semibold mb-2">{t(`howTo.steps.${step.key}.title`)}</h3>
                <p className="text-muted-foreground text-sm">{t(`howTo.steps.${step.key}.description`)}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-amber-900/30 via-black to-blue-900/20" />
        <div className="container mx-auto px-4 relative z-10">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/portal"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-amber-600 to-yellow-700 hover:from-amber-700 hover:to-yellow-800 rounded-full font-semibold transition-all shadow-lg shadow-amber-500/25"
              >
                <Wand2 size={20} />
                {t("cta.button")}
                <ArrowRight size={18} />
              ...
              <Link
                href="/cursos"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-secondary hover:bg-white/10 border border-white/20 rounded-full font-semibold transition-colors"
              >
