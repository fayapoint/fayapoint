  ...
  if (carregando) {
    return (
      <div className="flex items-center gap-2 rounded-xl border border-sky-500/20 bg-sky-500/[0.04] p-4 text-sm text-muted-foreground">
        <Loader2 className="h-4 w-4 animate-spin" /> Medindo o que o Brasil pergunta agora…
      </div>
    );
  }
    ...
    <div className="rounded-xl border p-4" style={{ borderColor: `${cor}33`, background: `${cor}0a` }}>
      <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
        <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider" style={{ color: cor }}>
          <Radar className="h-3.5 w-3.5" /> Pauta de hoje · {dados.nicho.label}
        </p>
        <button
          onClick={buscar}
      ...
      <p className="mb-3 text-[11.5px] leading-snug text-muted-foreground">
        {dados.personalizado ? (
          <>
            O que quem trabalha com <strong className="text-foreground">{dados.areaLabel}</strong> está procurando sobre IA agora —
            medido no autocomplete do Google e do YouTube.
          </>
        ) : (
          <>
            Recorte do Brasil inteiro.{" "}
            <strong className="text-foreground">Escolha sua área na aba Persona</strong> e esta lista passa a ser a da sua profissão.
          </>
        )}
      </p>
        ...
        {dados.pautas.map((p) => (
          ...
          >
            <span className="min-w-0 flex-1">
              <span className="block text-[12.5px] font-semibold capitalize text-foreground">{p.termo}</span>
              <span className="mt-0.5 flex flex-wrap items-center gap-1.5">
                ...
                >
                  ...
                  {p.so_video ? "demanda de vídeo" : p.canais === "web+yt" ? "nos dois canais" : "busca"}
                </span>
                <span className="text-[10px] text-muted-foreground">{p.formato}</span>
              </span>
            </span>
